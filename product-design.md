# Prime Machine: Product Design Document

## 1. Overview

**Prime Machine** is a specialized software platform designed to modernize and streamline the process of cash prize contributions for community bike races. The system provides a centralized hub for race organizers to manage events and for the cycling community to contribute to prize pools, known as "preems."
...
The platform is built around three core entities: Race Series, Races, and Preems. These concepts represent the fundamental data models of the application.

### 2.1 Race Series

A Race Series is a collection of related races, often managed by a single organizer or organization.

- **Fields:**
  - `seriesId`: A unique identifier for the series.
  - `organizerId`: A reference to the Race Organizer who owns the series.
  ...
  - `startDate`: The date the first race in the series begins.
  - `endDate`: The date the last race in the series ends.

### 2.2 Race

A Race is a single competitive event that belongs to a Race Series.

- **Fields:**
  - `raceId`: A unique identifier for the race.
  - `seriesId`: A reference to the parent Race Series.
  - `status`: The current state of the race: "Upcoming", "Live", or "Finished".
  - `name`: The specific name of the race (e.g., "Tuesday Night Crit").
  ...
  - `location`: The specific address or venue of the race.
...
- **Fields:**
  - `preemId`: A unique identifier for the preem.
  - `raceId`: A reference to the Race this preem is associated with.
  ...
  - `sponsorInfo`:
    - For a "One-Shot" preem, this holds the `userId` of the single contributor.
  ...
  - `contributionHistory`: A list of all individual contributions, where each entry contains:
    - `contributorId`: The `userId` of the contributor (or null if anonymous).
...
- **Event Management Screen:**
  - **Section 1: Upcoming Events:** A list of cards for each upcoming Race or Race Series.
    - **Card Details:** `race.name` or `series.name`, `race.dateTime` or `series.startDate`, `race.location`.
...
- **Section 1: Race Header:**
  ...
  - **Details List:** `race.dateTime`, `race.location`, `race.category`.
- **Section 2: Preems List:** A real-time table of all preems. The `preem.name` in each row links to the **Preem Detail Page**.
  - **Table Columns:**
    ...
    - **Sponsored By:** Derived from `preem.sponsorInfo`.
...
- **Section 2: Prize Pool & Sponsor:**
  ...
  - A "Sponsored by" section derived from `preem.sponsorInfo`.
...
- **Section 4: Contribution History:** A detailed, real-time table of every contribution from `preem.contributionHistory`.
  - **Table Columns:**
    - **Contributor:** Profile photo and username (or "Anonymous") from `contribution.contributorId`.
    - **Amount:** `contribution.amount`.
    - **Date:** `contribution.date`.
