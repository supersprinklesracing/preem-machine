...
const PreemDetailPage: React.FC<{ params: { id: string } }> = ({ params }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const findPreem = () => {
    for (const series of raceSeries) {
      for (const race of series.races) {
        const preem = race.preems.find((p) => p.id === params.id);
  ...
  const result = findPreem();

  if (!result) {
    return <div>Preem not found</div>;
  }

  const { preem, race } = result;
...
export default PreemDetailPage;
