...
import { ENV_USES_HTTPS } from '@/env/env';
...
export async function createOnboardingLink(
  ...
  try {
    const host = ENV_USES_HTTPS
      ? `https//${process.env.NEXT_PUBLIC_ORIGIN}`
      : 'http://localhost:3000';
    const params: Stripe.AccountLinkCreateParams = {
      account: accountId,
      refresh_url: `${host}/manage/organization/${organizationId}/edit`,
      return_url: `${host}/manage/organization/${organizationId}/edit`,
