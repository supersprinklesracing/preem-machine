...
const _getDocSnap = async <T>(
  ...
  return await db
    .doc(path)
    .withConverter(genericConverter<T>()) // The fix is on this line
    .get();
};
...
export const getOrganizationWithSeries = async (
  ...
  const snap = await organizationDoc.ref
    ...
    .get();
  for (const doc of snap.docs) {
    result.children.push(await getEventsForSeries(doc));
  }
  return result;
};
...
export const getRenderablePreemDataForPage = cache(async (path: string) => {
...
export const getRenderableRaceDataForPage = cache(async (path: string) => {
...
export const getRenderableSeriesDataForPage = cache(async (path: string) => {
...
// ...
export const getRenderableEventDataForPage = cache(async (path: string) => {
...
export const getRenderableUserDataForPage = cache(async (path: string) => {
  const db = await getFirestore();
  ...
  const contributionsSnap = await db
    .collectionGroup('contributions')
    .where('contributor.id', '==', path.split('/').pop())
    .withConverter(genericConverter<Contribution>())
    .get();
