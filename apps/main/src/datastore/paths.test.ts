...
describe('paths', () => {
  ...
  describe('isDocPath', () => {
    it('should return true for valid doc paths', () => {
      ...
      expect(isDocPath('series/series-id')).toBe(true);
      expect(isDocPath('users/user-id')).toBe(true);
      expect(isDocPath('events/event-id')).toBe(true);
      expect(isDocPath('races/race-id')).toBe(true);
      expect(isDocPath('preems/preem-id')).toBe(true);
      expect(isDocPath('contributions/contribution-id')).toBe(true);
    });
    ...
    it('should return false for paths with empty segments', () => {
      ...
      expect(isDocPath('organizations/')).toBe(false);
      // consecutive slashes
      expect(isDocPath('organizations//org-id')).toBe(false);
    });
  });
  ...
  describe('isCollectionPath', () => {
    it('should return true for valid collection paths', () => {
      ...
      expect(isCollectionPath('series')).toBe(true);
    ...
    it('should return false for invalid collection paths', () => {
      ...
      expect(isCollectionPath('organizations/org-id/series')).toBe(false);
      expect(isCollectionPath('users/user-id')).toBe(false);
    });

    it('should return false for paths with empty segments', () => {
      // trailing slash
      expect(isCollectionPath('organizations/')).toBe(false);
      // consecutive slashes
  ...
  describe('getCollectionPathFromSearchParams', () => {
    it('should return a CollectionPath for a valid searchParams', () => {
      const searchParams = { path: 'series' };
      expect(getCollectionPathFromSearchParams(searchParams)).toBe('series');
    });
  ...
  describe('isUrlPath', () => {
    ...
    it('should return false for invalid url paths', () => {
      expect(isUrlPath('a/b/c/d/e/f/g/h')).toBe(false);
  ...
  describe('toDocPath', () => {
    it('should convert a URL path to a doc path', () => {
      ...
      const expectedDocPath = 'events/event-1';
